...
__global__ void rotateImageKernel(const unsigned char* input, unsigned char* output,
    ...
    // Shared memory to store input block
    __shared__ unsigned char sharedMemory[BLOCK_SIZE][BLOCK_SIZE];

    // Calculate global coordinates
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;

    // Calculate local thread coordinates within the block
    int localX = threadIdx.x;
    int localY = threadIdx.y;

    // Load data to shared memory
    if (x < width && y < height) {
        sharedMemory[localY][localX] = input[y * width + x];
    }
    else {
        sharedMemory[localY][localX] = 0; // Handle boundary conditions
    }
    ...
    if (x < width && y < height) {
        ...
        if (srcX >= 0 && srcX < width && srcY >= 0 && srcY < height) {
            // Map rotated coordinates back to the block
            int blockSrcX = srcX - blockIdx.x * blockDim.x;
            int blockSrcY = srcY - blockIdx.y * blockDim.y;

            // Use shared memory if within block bounds; otherwise, fallback to global memory
            if (blockSrcX >= 0 && blockSrcX < BLOCK_SIZE && blockSrcY >= 0 && blockSrcY < BLOCK_SIZE) {
                output[y * width + x] = sharedMemory[blockSrcY][blockSrcX];
            }
            else {
...
int main()
{
    ...
    std::string outputFile = "rotated_optimized_output.pgm";
